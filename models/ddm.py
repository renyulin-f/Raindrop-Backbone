...
def noise_estimation_loss(model, x0, t, e, b):
    ...
    x = x0[:, 3:, :, :] * a.sqrt() + e * (1.0 - a).sqrt()              # (B,6,64,64) 这里的6前三个通道是LQ, 后三个通道是HQ
...
class DenoisingDiffusion(object):
    ...
    def compute_x0(self, x_t, e, betas, t):
        # 计算alpha_t的积累值（即 \bar{\alpha}_t）
        alpha = 1.0 - betas
        alpha_hat = torch.cumprod(alpha, dim=0)
        
        # 获取当前时间步t的 \bar{\alpha}_t 和 1 - \bar{\alpha}_t
        alpha_t = alpha_hat[t]  # \bar{\alpha}_t
        one_minus_alpha_t = 1.0 - alpha_t  # 1 - \bar{\alpha}_t

        # 使用模型预测的噪声e来反推x0
        # x0 = (xt - sqrt(1 - \bar{\alpha}_t) * e) / sqrt(\bar{\alpha}_t)
        x0 = (x_t - torch.sqrt(one_minus_alpha_t) * e) / torch.sqrt(alpha_t)
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                if self.step > self.config.training.n_iters: #Setting to 500,000
                    break
                # print(i,x.shape,y)
                ...
                if self.args.test_set in ['RDiffusion', 'Raindrop_DiT']:
                    if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                        checkpoint_path = os.path.join('Param_Patch_size_2_mul/'+ self.config.data.dataset +'/' + self.model_name + '_ddpm_epoch' + str(epoch)+'step' + str(self.step))
                    ...
                    if (epoch+1) % self.config.training.snapshot_freq  == 0:
                        checkpoint_path = os.path.join('Param_new/', self.config.data.dataset + '/' + self.model_name +'/'+'epoch'+str(epoch + 1))
                        utils.logging.save_checkpoint({
                            'epoch': epoch + 1,
                            'step': self.step,
                            'state_dict': self.model.state_dict(),
