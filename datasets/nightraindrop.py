...
class NightRaindrop:
    ...
    def get_loaders(self, parse_patches=True, validation='raindrop'):
        ...
        val_dataset = NightRaindropDataset(dir=os.path.join(self.config.data.data_val_dir),
                                      n=self.config.training.patch_n,
                                      patch_size=self.config.data.image_size,
                                      transforms=self.transforms,
...
#         '00173','00174','00176','00181','00184','00185','00186','00187','00188','00189',
#         '00190','00191','00192','00193','00194','00195','00196','00197'] 
#         self.trainlist_drop =  [x for x in self.list if x not in self.testlist]
#         sample_percentage = 0.3  # 30% of the trainlist
#         trainlist_size = int(len(self.trainlist_drop) * sample_percentage)
#         self.trainlist_blur = random.sample(self.trainlist_drop, trainlist_size)
#         print('-trainlis (Drop) -',self.trainlist_drop)
#         print('-trainlis (Blur) -',self.trainlist_blur)
#         print('-testlist (Not Used)-',self.testlist)
...
#                                         patch_size=self.config.data.image_size,
#                                         transforms=self.transforms,
#                                         filelist_drop=self.trainlist_drop,
#                                         filelist_blur=self.trainlist_blur,
#                                         parse_patches=parse_patches)
...
# class NightRaindropDataset(torch.utils.data.Dataset):
#     def __init__(self, dir, patch_size, n, transforms, filelist_drop=None, filelist_blur=None, parse_patches=True):
#         super().__init__()
#         print('-dir-',dir)
...
#         input_names = []
#         gt_names = []
#          # Add the filelist_drop:
#         for i in range(len(filelist_drop)):
#             inpdir = os.path.join(self.dir, 'Drop',filelist_drop[i])
#             gtdir = inpdir.replace('/Drop/','/Clear/')
#             # print(inpdir,gtdir)
#             listinpdir = sorted(os.listdir(inpdir))
#             for j in range(len(listinpdir)):
...
#         # Add the filelist_blur:
#         for i in range(len(filelist_blur)):
#             inpdir = os.path.join(self.dir, 'Blur',filelist_blur[i])
#             gtdir = inpdir.replace('/Blur/','/Clear/')
#             # print(inpdir,gtdir)
#             listinpdir = sorted(os.listdir(inpdir))
#             for j in range(len(listinpdir)):
#                 input_names.append(os.path.join(inpdir, listinpdir[j]))
